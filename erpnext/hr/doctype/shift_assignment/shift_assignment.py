...
def get_shifts_for_date(employee: str, for_timestamp: datetime) -> List[Dict[str, str]]:
	...
	return (
		...
		.where(
			...
			and (assignment.docstatus == 1)
			and (assignment.status == "Active")
			and (assignment.start_date <= getdate(for_timestamp.date()))
			and (
				Criterion.any(
					[
						assignment.end_date.isnull(),
						(assignment.end_date.isnotnull() and (getdate(for_timestamp.date()) < assignment.end_date)),
					]
				)
			)
		)
